...
@Component({
  ...
  template: `
    <div *ngIf="message?.type === 'system'; else chat" class="flex justify-center">
      <span class="rounded-full bg-muted px-4 py-2 text-sm text-foreground-secondary">{{ message?.text }}</span>
    </div>
    <ng-template #chat>
      <div class="flex gap-3" [class.flex-row-reverse]="message?.isSelf">
        ...
        <div class="max-w-[78%]" [class.text-right]="message?.isSelf">
          <p *ngIf="!message?.isSelf" class="mb-1 text-sm text-foreground-secondary">{{ message?.sender }}</p>
          <div class="rounded-2xl px-4 py-3" [class.bg-primary]="message?.isSelf" [class.bg-card]="!message?.isSelf">
            {{ message?.text }}
          </div>
          <p class="mt-1 text-xs text-foreground-muted">{{ message?.time }}</p>
        </div>
      </div>
    </ng-template>
  `,
