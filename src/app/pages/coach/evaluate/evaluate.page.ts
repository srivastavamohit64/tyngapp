        ...
        <div class="px-5 pt-4 space-y-6">
            ...
            <div class="space-y-2.5">
              <button
                *ngFor="let player of players"
                ...
                [style.backgroundColor]="selectedPlayer().id === player.id ? 'rgba(var(--app-primary-rgb),0.08)' : 'white'"
                [style.border]="selectedPlayer().id === player.id ? '2px solid var(--app-primary)' : '2px solid #F3F4F6'"
                [style.boxShadow]="selectedPlayer().id === player.id ? '0 2px 12px rgba(var(--app-primary-rgb),0.18)' : '0 1px 4px rgba(0,0,0,0.05)'"
              >
                <div class="w-12 h-12 rounded-full bg-gradient-to-br from-[var(--app-primary)] to-[var(--app-primary-to)] flex items-center justify-center text-2xl shadow-sm">
                  {{ player.avatar }}
                </div>
                <div class="flex-grow">
                ...
                <ion-icon *ngIf="selectedPlayer().id === player.id" name="checkmark-circle" class="text-[var(--app-primary)] text-xl"></ion-icon>
              </button>
            </div>
          </div>
            ...
            <div class="space-y-6">
              <div *ngFor="let cat of categories">
                <div class="flex items-center gap-2 mb-3">
                  <ion-icon [name]="cat.icon" class="text-[var(--app-primary)] text-lg font-bold"></ion-icon>
                ...
                <div class="flex gap-2">
                  <button
                    *ngFor="let rating of [1, 2, 3, 4, 5]"
                    ...
                    [style.backgroundColor]="getRatingVal(cat.id) >= rating ? 'var(--app-primary)' : '#F3F4F6'"
          ...
          <div>
            ...
            <textarea
              ...
              class="w-full px-4 py-4 bg-white rounded-2xl border-2 border-[#F3F4F6] focus:border-[var(--app-primary)] focus:outline-none transition-colors min-h-[100px] text-[13px] font-semibold text-[#111827] outline-none"
            ></textarea>
          </div>
          ...
          <button
            ...
            class="w-full bg-[var(--app-primary)] text-[#111827] py-4 rounded-2xl transition-colors flex items-center justify-center gap-2 border-none font-black text-[15px] shadow-lg shadow-[var(--app-primary)]/30"
          >
